...
def plotDataPoints(X_train, y_train, X_test, y_test, baseName, iteration, phase, originalLabels):
    ...
    # Plotando os dados de teste com legenda por classe
    ...
    for i, class_label in enumerate(classes_test):
        plt.scatter(X_test[y_test == class_label, 0], X_test[y_test == class_label, 1],
                    label=f'Test - {originalLabels[class_label]}', s=100, edgecolor='k', marker='x', cmap='coolwarm')
    ...
    plt.savefig(
        f"Resultados_BayesianGaussianMixture/{baseName}/Distribuicao_Dados_{baseName}_{phase}_Iteration_{iteration}.png")
